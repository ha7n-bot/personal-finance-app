...
export default async function TransactionsPage({ searchParams }: { searchParams: Promise<{ q?: string; type?: string }> }) {
  ...
  return <div><div className="page-heading-row"><div><span className="eyebrow">سجل أموالك</span><h1>العمليات</h1><p className="muted">سجّل الدخل والمصروف، وحدد إن كان مرة واحدة أو شهريًا.</p></div></div>
    ...
    <div className="page-grid transactions-layout"><section className="card registered-transactions">{rows.map((row) => { const income = row.type === "INCOME"; const account = income ? row.destinationAccount : row.sourceAccount; return <div className="transaction-row" key={row.id}><div className="transaction-copy"><strong>{row.description || (income ? "دخل" : "مصروف")}</strong><span>{row.category?.name || "بدون تصنيف"} · {account?.name || "بدون حساب"} · {row.occurredAt.toLocaleDateString("ar-SA")}</span></div><strong className={`number transaction-amount ${income ? "income" : "expense"}`}>{income ? "+" : "−"}{formatSAR(row.amount)}</strong><form action={deleteTransaction}><input type="hidden" name="id" value={row.id}/><button className="row-action">حذف</button></form></div>; })}{!rows.length ? <p className="muted panel-empty">لا توجد عمليات مطابقة.</p> : null}</section><TransactionForm accounts={accounts} categories={categories}/></div>
  </div>;
}
