...
export async function addTransaction(formData: FormData) {
  ...
  if (!parsed.success) throw new Error("تحقق من بيانات العملية");
  ...
  if (!account) throw new Error("أضف حسابًا صالحًا قبل حفظ العملية");
  if (parsed.data.categoryId && !category) throw new Error("التصنيف غير صالح");
  if (category && category.kind !== parsed.data.type) throw new Error("التصنيف لا يطابق نوع العملية");
