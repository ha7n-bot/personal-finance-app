...
self.addEventListener("fetch", (event) => {
  ...
  if (event.request.mode === "navigate") { event.respondWith(fetch(event.request).catch(() => caches.match("/demo"))); return; }
  const url = new URL(event.request.url);
  const safe = url.origin === self.location.origin && (url.pathname.startsWith("/_next/static/") || url.pathname.startsWith("/icons/") || url.pathname === "/manifest.webmanifest");
  if (!safe) return;
